...
instance ToPEM SignedCertificate where -- type SignedCertificate = SignedExact Certificate
  toPEM c = PEM
    { pemName = "CERTIFICATE"
    , pemHeader = []
    , pemContent = X509.encodeSignedObject c
    }
    ...
    where
      ...
      f ExtensionRaw{extRawOID, extRawCritical, extRawContent} = fromString (show extRawOID) .= show xs
        where
          ...
          -- ^ TODO: actually parse the extRawContent
